...
class MainRecommender:
    ...
    def __init__(self, data, weighting=True):
        ...
        item_privat = item_features['brand'] == 'Private'
        item_id_to_ctm = {}
        for i in range (len(item_features)):
            item_id_to_ctm [item_features['item_id'][i]] = item_privat[i]
                 
        self.item_id_to_ctm = item_id_to_ctm
        #НЕ СОВСЕМ ПОНЯТНО... item_fucher  ведь нет на входе 
    ...
    def get_similar_users_recommendation(self, user, N=5):
        ...
        # Находим пять похожих users в список similar_users
        M = N+1
        similar_user_m = model.similar_users(userid_to_id[10], M)
        similar_users = []
        for i in range(1,len(similar_user_m)):
            similar_users.append(a[i][0])
